...
knitr::kable(data.frame(Proverb, Opposite))
```
...
Table: **Arguments for Rational Choice**
...
```{r, include=FALSE}
S <- simulation(odd_nums = seq(1, 101, 2), params = c(10, 4))
err1 <- apply(S, MARGIN = 2, function(x) x < 0.5) %>% colMeans()
S <- simulation(odd_nums = seq(1, 101, 2), params = c(10, 3))
err2 <- apply(S, MARGIN = 2, function(x) x < 0.5) %>% colMeans()
S <- simulation(odd_nums = seq(1, 101, 2), params = c(10, 2))
err3 <- apply(S, MARGIN = 2, function(x) x < 0.5) %>% colMeans()
S <- simulation(odd_nums = seq(1, 101, 2), params = c(10, 1))
err4 <- apply(S, MARGIN = 2, function(x) x < 0.5) %>% colMeans()
```

```{r, echo=FALSE, fig.width=8, fig.height=3}
df <- tibble(N = seq(1, 101, 2), 
             "beta(10, 4)" = err1, "beta(10, 3)" = err2, 
             "beta(10, 2)" = err3, "beta(10, 1)" = err4)
...
error_rate_plot <- df %>% 
  ...
  geom_point(size = 0.5) +
  geom_line() +
  scale_y_continuous(label = scales::percent, breaks = seq(0, 0.3, 0.05)) +
  scale_x_continuous(breaks = seq(0, 100, 20)) +
  scale_color_viridis_d() +
  labs(y = "error rate", x = "jury size") +
...
```{r, echo=FALSE}
long_term <- c(round(pbeta(0.5, 10, 1), 3) * 100,
               round(pbeta(0.5, 10, 2), 3) * 100,
               round(pbeta(0.5, 10, 3), 3) * 100,
               round(pbeta(0.5, 10, 4), 3) * 100) %>% 
  rev()
...
for (i in seq_along(long_term)) {
  cat("A ", prior[i], " prior converges to: ", long_term[i], "% by increasing jury size\n", sep = "")
}
```
